class Game {
  constructor(canvas, levelData, levelIndex) {
    this.canvas = canvas;
  ...
  update() {
      ...
      this.blueCircles.forEach(bc => {
        if (bc.active && p.active && p.checkCollision(bc)) {
          ...
          showOverlay("Game Over", [
            { label: "Retry", onClick: () => new Game(this.canvas, this.levelData, this.levelIndex).gameLoop() },
            { label: "Back", onClick: showLevelSelect }
          ]);
        }
      });
    });
    ...
    if (this.score >= this.levelData.targetHits) {
      const nextLevel = this.levelIndex + 1;
      this.running = false;
      setTimeout(() => {
        showOverlay(`You beat Level ${this.levelIndex + 1}!`, [
          { label: "Next Level", onClick: () => new Game(this.canvas, LEVELS[nextLevel], nextLevel).gameLoop() },
          { label: "Retry", onClick: () => new Game(this.canvas, this.levelData, this.levelIndex).gameLoop() },
          { label: "Back", onClick: showLevelSelect }
        ]);
      }, 100);
